...
namespace ProspectManagement.Core.Services
{
    public class ActivityService: IActivityService
    {
        ...
        public async Task<List<Activity>> GetActivitiesForProspectAsync(int prospectId, int page, int pageSize)
        {
            try
            {
                var authResult = await _authenticator.AuthenticateUser(Constants.PrivateKeys.ProspectMgmtRestResource);
                return await _activityRepository.GetProspectActivitiesAsync(authResult.AccessToken, prospectId, page, pageSize);
            }
