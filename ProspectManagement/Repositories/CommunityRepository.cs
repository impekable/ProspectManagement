...
namespace ProspectManagement.Core.Repositories
{
    public class CommunityRepository : BaseRepository, ICommunityRepository
    {
        ...
        {
            return await GetDataObjectFromAPI<List<Community>>(string.Format(_devUri + "Communities?Area={0}&ActiveOnly={1}", divisionCode, activeOnly), accessToken);
        }

        public async Task<List<Community>> GetCommunitiesBySalespersonAsync(int salespersonId, string accessToken)
        {
			return await GetDataObjectFromAPI<List<Community>>(string.Format(_devUri + "Salesperson/{0}/Communities", salespersonId), accessToken);
        }
    }
}
